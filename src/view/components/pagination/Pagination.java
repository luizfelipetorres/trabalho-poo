...
public class Pagination extends JPanel {
    ...
    public void setPaginationItemRender(DefaultPaginationItemRender defaultPaginationItemRender) {
    ...
    private void changePage(int current, int totalPage) {
        ...
        if(current > 1) {
        	JButton cmdPrev = DefaultPaginationItemRender.createPaginationItem("Previous", false ,page.isPrevious());
        ...
        for (Object item : page.getItems()) {
            ...
            if (item instanceof Integer && Integer.valueOf(item.toString()) == page.getCurrent()) {
            	 cmd = DefaultPaginationItemRender.createPaginationItem(item,true ,isEnable(item));
            }else {
            	cmd = DefaultPaginationItemRender.createPaginationItem(item,false ,isEnable(item));
            }
        ...
        if(current < totalPage) {
        	JButton cmdNext = DefaultPaginationItemRender.createPaginationItem("Next", false,page.isNext());
