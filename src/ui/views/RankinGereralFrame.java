...
public class RankinGereralFrame extends JPanel {
	...
	private void  startup(){
		...
		photoPersonal= new JLabelRound[limit];
		...
		for (int i = 0; i < limit; i++) {
			...
			photoPersonal[i] = new JLabelRound();
	...
	private void loadData(int page) throws IOException {
		...
		for (int i = 0; i < limit; i++) {
			...
			Format.image(playerMatch.getPlayer().getFile(),photoPersonal[i]);
			...
			duration[i].setText(Format.hours(playerMatch.getDuration()));
