...
import ui.components.DefaultRankinGeneralItem;
...
public class RankinGereralFrame extends JPanel {
	...
	private void loadData(int page) throws IOException {
		...
		for (int i = 0; i < limit; i++) {
			...
			Format.image(playerMatch.getPlayer().getFile(),defaultRankinGenerals.get(i).getPhotoPersonal());
			...
			defaultRankinGenerals.get(i).setDuration(Format.hours(playerMatch.getDuration()));
